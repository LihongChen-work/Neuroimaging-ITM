...
class model:
    ...
    def evaluate2(self,eval_data,operations,set,i):

        print('-------Evaluate2 on ' + set + '-------')

        evaluator = self.getEvaluator()
        n=0
        for x_dev in utils.generator(eval_data, operations.m_op, self.config, train=False):
            predicted_ner, actual_ner, predicted_rel, actual_rel, _, m_eval = self.sess.run(
                [operations.predicted_op_ner, operations.actual_op_ner, operations.predicted_op_rel,
                 operations.actual_op_rel, operations.score_op_rel, operations.m_op], feed_dict=x_dev)

            if self.config.evaluation_method == "relaxed":
                evaluator.add2(predicted_ner, actual_ner, predicted_rel, actual_rel, m_eval['BIO'],n,i)
                n = n+1
            else:
                evaluator.add2(predicted_ner, actual_ner, predicted_rel, actual_rel,n,i)
                n = n + 1

        if self.config.evaluation_method == "relaxed":
            evaluator.computeInfoMacro(printScores=True)
            if "other" in [x.lower() for x in
                           self.config.dataset_set_ec_tags]:
                return evaluator.getMacroF1scoresNoOtherClass()[2]
            else:
                return evaluator.getMacroF1scores()[2]
    ...
    def run(self):
        ...
        #perturb the inputs 干扰输出
        raw_perturb = tf.gradients(obj, embeddings_input)[0]  # [batch, L, dim]
        normalized_per=tf.nn.l2_normalize(raw_perturb, axis=[1, 2])
        perturb =self.config.alpha*tf.sqrt(tf.cast(tf.shape(input_rnn)[2], tf.float32)) * tf.stop_gradient(normalized_per)#干扰
        perturb_inputs = embeddings_input + perturb#干扰版输入
...
class operations():
    def __init__(self,train_step,obj, m_op, predicted_op_ner, actual_op_ner, predicted_op_rel, actual_op_rel, score_op_rel):
        ...
        self.train_step=train_step #训练步长
        self.obj=obj
        self.m_op = m_op
